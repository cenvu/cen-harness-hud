...
def ownership_aware_cleanup(
    ...
    On match: clears cen_codex_profile (launcher-owned) and sets
    cen_codex_summary = "quota —" (bridge source), consistent with the
    publisher's fail-closed UX. The fresh time_ns() sequence is monotonic for
    each source; because we only act while still owning the registration, no
    newer session's summary can exist on this pane at that moment (residual
    TOCTOU window is milliseconds and self-heals on the next event).
